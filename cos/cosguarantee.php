    ...
    if ($bulkAction == 'approve') {
        foreach ($selectedItems as $itemId) {
            ...
            if (isset($_SESSION['usertype'])) {
                $action = 'Approve the guarantee letter application of beneficiary ' . $itemId;
        ...
        foreach ($selectedItems as $itemId) {
            ...
            // log action
            if (isset($_SESSION['usertype'])) {
                $action = 'Reject the guarantee letter application of beneficiary ' . $itemId;
                $action_query = mysqli_query($connection, "INSERT INTO logs (username, usertype, action, logdate) VALUES ('" . $user_data['username'] . "', '" . $user_data['usertype'] . "', '$action', NOW())");
            }
        }
    }
    header("Location: cosguarantee.php"); // Redirect back to the main page after bulk action
}
...
function exportSelectedToExcel($selectedIds, $connection)
{
    ...
    header("Content-Disposition: attachment; filename=\"CosGuarantee_data.xls\"");
    ...
    foreach ($selectedIds as $id) {
        $id = intval($id);
        $stmt = $connection->prepare("SELECT * FROM guarantee WHERE id = ?");
        ...
        echo "<td>" . $row['gletter'] . "</td>";
        echo "<td>" . $row['amount'] . "</td>";
        echo "<td>" . $row['gstatus'] . "</td>";
        ...
        <div class="container">
            <div class="row">
                <div class="col-lg-10 col-12 text-center mx-auto">
                    <h2 class="mb-5"><b>List of Guarantee Letter Beneficiaries</b></h2>
                </div>
            </div>
        </div>
    </section>
