...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['bulk_action'])) {
    ...
    if ($bulkAction == 'approve') {
        foreach ($selectedItems as $itemId) {
            $query = mysqli_query($connection, "UPDATE burial SET bstatus = 'Approved' WHERE id = $itemId");

            // log action
            if (isset($_SESSION['usertype'])) {
                $action = 'Approve the burial application of beneficiary ' . $itemId;
        ...
        foreach ($selectedItems as $itemId) {
            $query = mysqli_query($connection, "UPDATE burial SET bstatus = 'Rejected' WHERE id = $itemId");

            // log action
            if (isset($_SESSION['usertype'])) {
                $action = 'Reject the burial application of beneficiary ' . $itemId;
                $action_query = mysqli_query($connection, "INSERT INTO logs (username, usertype, action, logdate) VALUES ('" . $user_data['username'] . "', '" . $user_data['usertype'] . "', '$action', NOW())");
            }
        }
    }
    header("Location: cosburial.php"); // Redirect back to the main page after bulk action
}
...
function exportSelectedToExcel($selectedIds, $connection)
{
    ...
    header("Content-Disposition: attachment; filename=\"CosBurial_data.xls\"");
