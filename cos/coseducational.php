...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['bulk_action'])) {
    ...
    if ($bulkAction == 'approve') {
        foreach ($selectedItems as $itemId) {
            $query = "UPDATE educational SET estatus = 'Approved' WHERE id = $itemId";
            mysqli_query($connection, $query);

            // log action
            if (isset($_SESSION['usertype'])) {
                $action = 'Approve the Educational letter application of beneficiary ' . $itemId;
        ...
        foreach ($selectedItems as $itemId) {
            $query = "UPDATE educational SET estatus = 'Rejected' WHERE id = $itemId";
            mysqli_query($connection, $query);

            // log action
            if (isset($_SESSION['usertype'])) {
                $action = 'Reject the Educational letter application of beneficiary ' . $itemId;
                $action_query = mysqli_query($connection, "INSERT INTO logs (username, usertype, action, logdate) VALUES ('" . $user_data['username'] . "', '" . $user_data['usertype'] . "', '$action', NOW())");
            }
        }
    }
    header("Location: coseducational.php"); // Redirect back to the main page after bulk action
}
...
function exportSelectedToExcel($selectedIds, $connection)
{
    ...
    header("Content-Disposition: attachment; filename=\"CosEducational_data.xls\"");
    ...
    foreach ($selectedIds as $id) {
        $id = intval($id);
        $stmt = $connection->prepare("SELECT * FROM educational WHERE id = ?");
        ...
        <div class="container">
            <div class="row">
                <div class="col-lg-10 col-12 text-center mx-auto">
                    <h2 class="mb-5"><b>List of Educational Beneficiaries</b></h2>
                </div>
            </div>
        </div>
    </section>
