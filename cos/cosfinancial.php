...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['bulk_action'])) {
    $selectedItems = isset($_POST['selected_ids']) ? $_POST['selected_ids'] : [];
    $bulkAction = $_POST['bulk_action'];

    if ($bulkAction == 'approve') {
        foreach ($selectedItems as $itemId) {
            $query = "UPDATE financial SET fstatus = 'Approved' WHERE id = $itemId";
            mysqli_query($connection, $query);

            // log action
            if (isset($_SESSION['usertype'])) {
                $action = 'Approve the financial application of beneficiary ' . $itemId;
                $action_query = mysqli_query($connection, "INSERT INTO logs (username, usertype, action, logdate) VALUES ('" . $user_data['username'] . "', '" . $user_data['usertype'] . "', '$action', NOW())");
            }
        }
    } elseif ($bulkAction == 'reject') {
        foreach ($selectedItems as $itemId) {
            $query = "UPDATE financial SET fstatus = 'Rejected' WHERE id = $itemId";
            mysqli_query($connection, $query);

            // log action
            if (isset($_SESSION['usertype'])) {
                $action = 'Reject the financial application of beneficiary ' . $itemId;
                $action_query = mysqli_query($connection, "INSERT INTO logs (username, usertype, action, logdate) VALUES ('" . $user_data['username'] . "', '" . $user_data['usertype'] . "', '$action', NOW())");
            }
        }
    }
    header("Location: cosfinancial.php"); // Redirect back to the main page after bulk action
}
...
function exportSelectedToExcel($selectedIds, $connection)
{
    ...
    header("Content-Disposition: attachment; filename=\"CosFinancial_data.xls\"");
    ...
    foreach ($selectedIds as $id) {
        $id = intval($id);
        $stmt = $connection->prepare("SELECT * FROM financial WHERE id = ?");
...
<head>
    ...
    <title>Admin GL Beneficiaries</title>
            ...
            <div class="collapse navbar-collapse" id="navbarNav">
                <ul class="navbar-nav ms-auto">
                    <li class="nav-item ms-3">
                        <a class="nav-link custom-btn custom-border-btn btn" href="../cos/cos.php">Return</a>
                    </li>
                </ul>
            </div>
        </div>
        ...
        <div class="container">
            <div class="row">
                <div class="col-lg-10 col-12 text-center mx-auto">
                    <h2 class="mb-5"><b>List of Financial Beneficiaries</b></h2>
                </div>
            </div>
        </div>
    </section>
    ...
    <div class="container">
        ...
        <form action="" method="post">
            <div>
                ...
                <button name="bulk_action" class="btn btn-primary btn-sm" value="approve">Approve Selected</button>
                <button name="bulk_action" class="btn btn-danger btn-sm" value="reject">Reject Selected</button>
            ...
            <table class="table table-hover">
                ...
                <tr>
                    ...
                    <td>$row[pnum]</td>
                    <td>$row[financial]</td>
                    <td>$row[bfname] " . " $row[bmname] " . " $row[blname]</td>
                    <td>$row[amount]</td>
                    <td>$row[fstatus]</td>
